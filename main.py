...
@app.route('/questionsParsed', methods=["POST"])
...
def import_questions_redis2(fileCorrection, fileSujet):
    correction = ocr_World_Correction2(fileCorrection)
    questions = ocr_World_Questions2(fileSujet)
    correction.pop(0)
    for i in range(len(questions)):
        questions[i].append(correction[i])
        redisData = {}
...
def ocr_World_Correction2(fileCorrection):
    document = Document(fileCorrection)
    responses = []
    for row in document.tables[0].rows:
        el = []
        for cell in row.cells:
...
def ocr_World_Questions2(fileSujet):
    doc = docx.Document(fileSujet)  # Creating word reader object.
    data = ""
    fullText = []
    nbPoints = 0;
    for para in doc.paragraphs:
